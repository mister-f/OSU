...
void addHeap(DynArr *heap, TYPE node)
{
	...
	/* Percolate value up heap if necessary */
	currentPosition = sizeDynArr(heap) - 1;

	while (currentPosition != 0)
	{
		parentPosition = (currentPosition - 1) / 2;
		if (_smallerIndexHeap(heap, currentPosition, parentPosition) == currentPosition)
		{
			swapDynArr(heap, currentPosition, parentPosition);
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rightChild < max)
	{
		/* if there is a right child, there must be 2 children, find the smaller */
		smallChild = _smallerIndexHeap(heap, leftChild, rightChild);

		/* if child is smaller, swap with pos and re-adjust*/
		if (_smallerIndexHeap(heap, smallChild, pos) == smallChild)
		{
			swapDynArr(heap, smallChild, pos);
...
void removeMinHeap(DynArr *heap)
{
	...
	/* move last value in array to first position*/
...
void _buildHeap(DynArr *heap)
{
	...
	/* adjust heap starting at first non-leaf node */
	for (i = ((sizeDynArr(heap) / 2) - 1); i >= 0; i--)
	{
...
void sortHeap(DynArr *heap)
{
	...
	/* turn array into valid heap */
	_buildHeap(heap);

	for (i = (sizeDynArr(heap) - 1); i >= 0; i--)
	{
		/* move root node to last position */
		swapDynArr(heap, 0, i);

		/* adjust heap, excluding value just swapped and higher*/
